...
model = load_model(os.path.join(BASE_DIR, 'models', 'final', 'stroke_final_model.h5'))
scaler = joblib.load(os.path.join(BASE_DIR, 'models', 'final', 'scaler.pkl'))

# Load feature names to correctly order input
import pickle
with open(os.path.join(BASE_DIR, 'data_processing', 'preprocessed_data.pkl'), 'rb') as f:
    meta = pickle.load(f)
